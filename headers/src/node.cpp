...
int numNodes;
int node_generationSize;
int node_symbolSize;
FieldSize node_fieldSize;
int nodeIndex;

////
// Define a struct to represent the input type
// struct NodeInput {
std::vector<std::vector<std::vector<uint8_t>>> nodeDataInput; // 3D input matrice for incoming data packets to nodes
...
std::vector<std::vector<std::vector<uint8_t>>> nodeKeyInput;  // 3D input matrice for assigned keys  to nodes
...
void setInput(int nodeIndex)
{
  ...
  int cnt = 0;
  for (int adjentNode = 0; adjentNode < adjacencyList[nodeIndex].size(); adjentNode++)
  {
    for (int count = 0; count < nodeOutput[adjentNode].size(); count++)
    {                                                                // change to a adjacencyList[adjentNode]
      nodeDataInput[nodeIndex][cnt] = nodeOutput[adjentNode][count]; // change to a adjacencyList[adjentNode]
      cnt = cnt + 1;
    };
  };
};
