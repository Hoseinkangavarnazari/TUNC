...
void simulation(Graph _topology, std::vector<std::vector<Graph::vertex_descriptor>> path_list, int _GG, int _G, int _fieldSize, int _packetSize, int _keypoolSize, int _keysetSize);
// ........................................................
int main()
{
  // generate topology
 ...
 // set the main simulation parameters
  ...
  int _G = 10;    //generationsize
  int _fieldSize = 256;
  int _packetSize = 100;
  int _keypoolSize = 15;
  int _keysetSize = 3;
  int Number_of_Pollution=3;
  int _bufferSize = 16;
...
void simulation(Graph _topology, std::vector<std::vector<Graph::vertex_descriptor>> path_list, int _GG, int _G, int _fieldSize, int _packetSize, int _keypoolSize, int _keysetSize, int Number_of_Pollution,int _bufferSize)
{
   // Get the number of nodes in the graph
    ...
    std::vector<std::vector<uint8_t>>zeroBufferMatrice(_bufferSize,std::vector<uint8_t>(_packetSize,0));
    ...
    // Convert the number of vertices to an int
    int number_of_nonsource_nodes= static_cast<int>(numVertices)-1;
    std::vector<int>ARvector(std::vector<int>(5,1));  // Size of AR vector will be updated
       ...
       //Ask for sending to next node in the path 
        ...
        else if (_topology[path_list[pathIndex][j]].type == "Intermediate")
        {
         ...
         if (_topology[path_list[pathIndex][j]].buffercounter == _bufferSize)
         {
          ...
          p1.arTreeVerifierNEW(generated_tree, _topology[path_list[pathIndex][j]].arVectorForPackets, NumberOfLayers, _topology[path_list[pathIndex][j]].keySet, private_key);
         }
...
/* check the packet through verification,checkNumber++, */
         ...
         if (_topology[path_list[pathIndex][j]].buffercounter == _bufferSize)
         {
          ...
          std::vector<int> ar_ver_output = p1.arTreeVerifierNEW(generated_tree, ARvector, NumberOfLayers, _topology[path_list[pathIndex][j]].keySet, private_key);
