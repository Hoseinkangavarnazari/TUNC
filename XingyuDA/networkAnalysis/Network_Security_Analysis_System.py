...
class KDC:
    ...
    def distribute_fix_num_keys_with_mhd(self, keyset_size: int, keypool_size: int):
        ...
        def pick_best_keyset(g: nx.Graph, node: int, keysets_possible_b: List[str], keypool_size) -> str:
            """
            ...
            keysets_with_mhd_b = find_keysets_with_mhds(g, node, keysets_possible_b, keypool_size)
        ...
        while len(assigned_group) < len(subgraph.nodes):
            ...
            keyset_chosen_b = pick_best_keyset(subgraph, node, keysets_possible_b, keypool_size)
...
def compare_time():
    ...
    bars = plt.bar(methods, times, width=0.3, color=[color_map[method] for method in methods])

    for bar, time in zip(bars, times):
        plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{time:.3f}', ha='center', va='bottom', fontsize=12)
...
def demo():
    g1 = NGraph()
    # g1.generate_ws_graph(num_nodes=10)
    ...
    kdc.distribute_fix_num_keys_with_mhd(keyset_size=9, keypool_size=18)
    g1.take_snapshot()
    g1.visualize()
